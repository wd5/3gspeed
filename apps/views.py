...
class PointsListJSON(View):
    def get(self, request):
        ...
        for point in points:
            ...
            point_json['ability_icon'] = point.get_abilitiy_icon()
            op = list()
            for item in point.get_operators_without_params():
                op.append(item.title)
            op.insert(0, 'Операторы')
            op.insert(1, 'Все')
            point_json['operators'] = op
